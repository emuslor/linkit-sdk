...
typedef struct
{
    uint8_t addr[LGATT_BD_ADDR_SIZE]; // The address of a Bluetooth 4.0 device. The structure of the address represents a 12-digit hexadecimal value.
}LGATTAddress;
...
typedef struct
{
    ...
    uint8_t value[LGATT_ATT_MAX_VALUE_LEN]; // The array of the attribute value.
}LGATTAttributeValue;
