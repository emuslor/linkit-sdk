...
// DESCRIPTION: This is a structure that defines the Bluetooth GATT service, characteristic and descriptor.
//   
// A Bluetooth GATT service is defined by an array of LGATTServiceInfo, the first
...
// Please see LGATTServerClass.loadService() for more details.          
 typedef struct
{
    ard_decl_type_enum          type; 		 // The type of this entry can be TYPE_SERVICE, TYPE_CHARACTERISTIC, TYPE_DESCRIPTOR, or TYPE_END.
    uint8_t                     uuid[37];  // The UUID string of this entry in canonical form representation, for example, "123e4567-e89b-12d3-a456-426655440000" plus the terminating NULL character. Please see http://en.wikipedia.org/wiki/Universally_unique_identifier#Definition for details.
    uint8_t                     isprimary; // This entry is valid only if it's type TYPE_SERVICE. Set value to 1 if this service is a primary service of the GATT profile which is discovered by the primary service. 
    ...
    LGATT_PERMISSION         permission;  // This is the bit field flag representing access permissions of this characteristic or descriptor. This entry is valid only if it's type TYPE_SERVICE or TYPE_DESCRIPTOR.
    int32_t                     triggered;  // This is reserved and it must be 0.
} LGATTServiceInfo;
...
{
    uint16_t handle; // The handle of the attribute and it's required in LGATTServer.sendIndication().                            
                    
    LGATTUUID uuid;  // The attribute UUID.
                      
    boolean failed;  // Indicates if the transaction failed or not.
...
{
    LGATTAddress addr; // The Bluetooth 4.0 device's address.
    boolean connected; // Indicates whether it has got a connection or lost a connection.
    LGATTConnection()
    {
...
public:
    ...
    // PARAMETERS
    //    value: The attribute value, refer to LGATTAttributeValue.
    //    attr_handle: The attribute handle. 
    //    failed: Confirms if the data has been properly received by sending ACK to central device. 
    ...
    // PARAMETERS
    //    value :        The attribute value, refer to LGATTAttributeValue. 
    //    attr_handle :  The attribute handle that is passed from
    //                   onCharacteristicAdded(). 
    //    needConfirm :  The request for response from central device. To the Bluetooth protocol, if it's set to true, it means indication; if it's set to false, it means notification.     //                   
    //                   
    // RETURNS
    //    true: Successful.
    //    false:  Failed.                                                        
...
protected:
    // DESCRIPTION:
    //    You need to override this method in order to define a Bluetooth GATT service.
    ...
    //    TYPE_DESCRIPTOR. Also, it must be terminated with an element of TYPE_END.
    // PARAMETERS
    ...
    //    connected: True means a connection is established, false means the central device is disconnected.
    // RETURNS
    //    true: Successful.
    //    false:  Failed.
...
public:
    ...
    // DESCRIPTION: 
    //   This function handles events for Bluetooth 4.0 and process all the requests that have been added to the event queue.
...
public:
    ...
    LGATTReadRequest()
    {
        ...
        // You can call this method in onRead() and callback to respond with failed result to the read request.
    boolean ackFail()
    {
        // sendAck(fail).      
        LGATTAttributeValue value;
        value.len = 0;
        return ((LGATTService *)_srv)->sendResponse(value, this->attr_handle, 1, this->trans_id); // The ACK.
    }
    ...
    {
        return ((LGATTService *)_srv)->sendResponse(value, this->attr_handle, 0, this->trans_id); // The ACK.
    }
}; 

// This class represents a write request from the central device.
...
public:
    ...
    boolean need_rsp;            // This indicates whether it is required to respond to the remote device or not.                                                                
    ...
    // You can call this method in onWrite() and callback to respond with a failed result to the central device.
    boolean ackFail()
    {
        // The sendAck(fail).      
        LGATTAttributeValue value;
    ...
    // You can call this method in onWrite() and callback to respond with a successful result to central device.
    boolean ackOK()
    {
        // The sendAck(OK).
        LGATTAttributeValue value;
